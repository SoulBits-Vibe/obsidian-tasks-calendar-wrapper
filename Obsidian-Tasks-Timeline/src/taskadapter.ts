...
function isParentPath(parent: string, path: string) {
    const normalizedParent = normalizeFilterPath(parent);
    const normalizedPath = normalizeFilterPath(path);
    if (normalizedParent.length === 0) return false;
    if (normalizedParent.length > normalizedPath.length) return false;
    const paths = normalizedPath.split('/');
    const parents = normalizedParent.split('/');
...
export class ObsidianTaskAdapter {
    ...
    pathsFilter(filter: string[]) {
        const normalizedFilter = filter.map(normalizeFilterPath).filter(path => path.length > 0);
        return (file: TFile) => {
            const fileName = file.path;
            return !normalizedFilter.some((path) => isParentPath(path, fileName));
        }
    }
